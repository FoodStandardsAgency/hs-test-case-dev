...
load("test-cases/food-contact-material/fcm_clean.RData")

wos <- fcm_clean %>% rename(abstract = abstract_text)
...
wos <- wos %>% 
  ...
  filter_at(., vars(title, abstract), any_vars(str_detect(., fixed("biofilm", ignore_case = TRUE))))
...
# only searched for one phrase ("biofilm"), with title relevancy filter

crsearchresult <- map2(as.character(sinces), as.character(tos), safely(crSearch, otherwise = "fail"), query = "biofilm")

crfcm <- transpose(crsearchresult) %>% 
  .$result %>% 
  .[. != "fail"] %>% 
  bind_rows() 

noab <- crfcm %>% filter(abstract == "") %>% pull(doi) 

abstracts <- map(noab, slowly(safely(getAbstract,otherwise = "fail"), rate = purrr::rate_delay(pause = 0.1)))
...
uimatch <- map_df(uilist$institution, function(x) fcmaff %>% 
...
fcm <- fcm %>% 
  ...
  select(doi, title, container, published, author, pubtype, researcharea, funder, affiliation, year, title_text, title_lemma,
...
#DBI::dbSendQuery(con, "DELETE FROM fcm")
DBI::dbAppendTable(con, "fcm", fcm)
